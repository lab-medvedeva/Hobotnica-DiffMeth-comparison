...
process_dataset <- function(smoothing, cores) 
{
    ...
    # Get DMC signature
    dmlTest = DMLtest(bsseq_stranded_filtered, group1= samples_group1, group2=samples_group2, smoothing=smoothing, ncores = cores)
    dmls <- callDML(dmlTest, p.threshold=1)
    dmls_fdr  <- dmls[which(dmls$fdr <= 0.05),]
    dmls_fdr_sorted<-dmls_fdr[order(dmls_fdr$fdr),]
    write.table(dmls_fdr_sorted, paste(dataset_name, method_name, "full", sep = "_"), sep = '\t')
    ...
    write.table(dmls_fdr_diff_cut, paste(dataset_name, method_name, "cut", sep = "_"), sep = '\t')

    # Get Hobotnica score
    dmls_fdr_sorted <- read.csv(paste(dataset_name, method_name, "cut", sep = "_"), sep = '\t')
...
# Process DSS without smoothing
process_dataset(smoothing=FALSE, cores=opt$cores)

# Process DSS with smoothing
process_dataset(smoothing=TRUE, cores=opt$cores)