...
gse <- getGEO('GSE143411', GSEMatrix=TRUE)
data <- gse[[1]]
data_matrix <- exprs(data)

ighv_mutation_status <- data$ighv_mutation_status
...
data_matrix_untreated <- data_matrix[,untreated]
...
result_untreated <- get_random_H(H_score_untreated, signature_truncated, ighv_mutation_status_untreated, data_matrix_untreated, signature_length, 96)
...
p <- ggplot(result_H_df, aes(x=H)) + 
  ...
  ggtitle("H-score distribution for GSE143411, Kulis et al. (2012) signature")+
  theme(plot.title = element_text(size = 12, face = "bold"),
        axis.title=element_text(size=10,face="bold"),
        axis.text = element_text(size = 10)) +
  coord_cartesian(clip = "off")

ggsave('plots//GSE143411_untreated_validation.png',  plot = p,  device = "png", width = 20,  height = 15,  units = "cm")
write.table(result_H_df,
  "plots//GSE143411_H.csv" ,
  quote=F,
  row.names=F,
  col.names = T,
  sep = '\t')
...
p<-ggplot(mds_df, aes(x=V1, y=V2, color=sample_type)) + geom_point(size = 3) +
  ...
  ggtitle("MDS for GSE143411, Kulis et al. (2012) signature")+
  theme(plot.title = element_text(size = 12, face = "bold"),
        axis.title=element_text(size=10,face="bold"),
        axis.text = element_text(size = 10),
...
ggsave(paste0("plots//GSE143411_untreated_validation_MDS.png"),  plot = p,  device = "png", width = 20,  height = 15,  units = "cm")
